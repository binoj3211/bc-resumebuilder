...
const TemplateSelector = ({ selectedTemplate, onSelectTemplate }) => {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 max-h-96 overflow-y-auto">
        {templates.map((template) => (
          ...
          >
            ...
            <div className={`w-full h-16 rounded mb-2 ${template.preview}`}>
              <div className="p-2 space-y-1">
                <div className="h-1 bg-gray-300 rounded w-3/4 opacity-60"></div>
            ...
            <div>
              <h4 className="font-medium text-gray-900 text-xs">{template.name}</h4>
              <p className="text-xs text-gray-500 mt-1 leading-tight">{template.description}</p>
            </div>
          </button>
        ))}
      </div>
